...
if [ "$OPENPLC_PLATFORM" = "win" ]; then
    ...
    if [ "$OPENPLC_DRIVER" = "sl_rp4" ]; then
        ...
        g++ -std=gnu++11 ezJSON.c cJSON.c tcp_spi.c *.cpp *.o -o openplc -I ./lib -pthread -fpermissive `pkg-config --cflags --libs libmodbus` -lasiodnp3 -lasiopal -lopendnp3 -lopenpal -w $ETHERCAT_INC
    fi
    if [ $? -ne 0 ]; then
        echo "Error compiling C files"
        echo "Compilation finished with errors!"
